...
potDepths = numpy.arange(-10, 2, 0.05)
#for n in potDepths:
#    n = round(n, 3)

rfoNames = ["KCurve", "KFound", "Potential", "PotentialPrecision", "InterestingPoints", "Wavefunctions"]
    ...
    for nameOfFile, depth, dir0 in zip(cfNames, potDepths, oNames):
        with open(nameOfFile,"w") as f:
            for l in lines:
                ...
                currLine = currLine.replace("DEPTH", "%.2f" % depth)
                f.write(currLine )
...
def computeRun(configFile):
    ...
    #with open("/dev/null", "w") as f:
        with open(os.devnull, "w") as f:
            ...
            subprocess.call(['./Compute2',  '--configFile', configFile], stdout=f, stderr=f)
    except:
